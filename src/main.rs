...
fn load_salary_data(file_path: &str) -> Vec<Salary> {
    ...
    if let Some(Ok(range)) = workbook.worksheet_range("Sheet1") {
        for row in range.rows().skip(1) {
            ...
            //let salary_date_parsed= NaiveDate::parse_from_str(&salary_date, "%d-%m-%Y").expect("Failed to parse salary date");
...
fn get_salary_status(salaries: &Vec<Salary>, emp_id: i32, current_month: NaiveDate) -> String {
    //let current_month_str = format!("{:02}-{}", current_month, current_year);
    // for salary in salaries {
    //     if salary.emp_id == emp_id && salary.salary_date.contains(&current_month) && salary.salary_status == "Credited" {
    ...
    for salary in salaries {
        // Parse the salary_date as a NaiveDate
        //println!("salary.salary_date-{}",salary.salary_date);
        let salary_date = NaiveDate::parse_from_str(&salary.salary_date, "%d-%m-%Y").unwrap_or_else(|_| {
        //     // If the date parsing fails, return a dummy date
             NaiveDate::from_ymd_opt(1900, 1, 1).expect("None")
        });
        //println!("sal{:?},{:?}",salary_date,current_month.month());
        // Check if the emp_id matches, and if the month and year of salary_date match current_month
        if salary.emp_id == emp_id
            && salary_date.month() == current_month.month()
            
        {
            // Check if the salary status is "Credited"
            if salary.salary_status == "Credited" {
                return "Credited".to_string();
            } else {
                return "Not Credited".to_string();
            }
        }
    }

    // If no match found, return "Not Credited"
    "Not Credited".to_string()
}
...
fn main() {
    ...
    // println!("salary_data_file_path-{}",args.salary_data_file_path);
    ...
    // Create a NaiveDate for the first day of the current month
    let current_month = NaiveDate::from_ymd_opt(current_date.year(), current_date.month(), 1).expect("None");
    ...
    .map_or("N/A".to_string(), |dept| dept.dept_title.clone());
        ...
        let salary_status = get_salary_status(&salaries, emp.emp_id, current_month);
